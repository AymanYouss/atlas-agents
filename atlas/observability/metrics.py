...
RUNS_TOTAL = Counter("atlas_runs_total", "Total runs started, by terminal status.", ["status"])
RUN_DURATION = Histogram(
    "atlas_run_duration_seconds",
    "End-to-end run duration.",
    buckets=(1, 5, 15, 30, 60, 120, 300, 600, 900),
)
STEPS_TOTAL = Counter("atlas_steps_total", "Plan steps executed, by outcome.", ["outcome"])
TOOL_CALLS_TOTAL = Counter(
    "atlas_tool_calls_total", "Tool invocations, by tool and outcome.", ["tool", "ok"]
)
TOOL_LATENCY = Histogram("atlas_tool_latency_seconds", "Tool call latency, by tool.", ["tool"])
